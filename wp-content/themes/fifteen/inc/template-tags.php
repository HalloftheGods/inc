...
function fifteen_posted_on_date() {
	$time_string = '<time class="entry-date published updated" datetime="%1$s">%2$s</time>';
	if ( get_the_time( 'U' ) !== get_the_modified_time( 'U' ) ) {
		$time_string = '<time class="entry-date published" datetime="%1$s">%2$s</time><time class="updated" datetime="%3$s">%4$s</time>';
	}
...
function fifteen_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		if ( $categories_list && fifteen_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( '<span class="cat-title">Categories</span> %1$s', 'fifteen' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ' ', 'fifteen' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( '<span class="tag-title">Tags</span> %1$s', 'fifteen' ) . '</span>', $tags_list );
		}
	}
...
function fifteen_comment( $comment, $args, $depth ) {
	...
	<li id="comment-<?php comment_ID(); ?>" <?php comment_class( empty( $args['has_children'] ) ? 'fifteen' : 'parent' ); ?>>
		<article id="div-comment-<?php comment_ID(); ?>" class="comment-body row">
			...
			?>
			<?php edit_comment_link( __( 'Edit', 'fifteen' ), '<span class="edit-link">', '</span>' ); ?>
			</div><!-- .comment-content -->
...
endif; // ends check for fifteen_comment()
