...
class fifteen_Recent_Posts extends WP_Widget {
	...
	if (isset($instance['title'])) :
		...
		<?php
         ...
         ?>
		         ...
		         <div class='rp-date'><?php the_time(__('M j, Y','fifteen')); ?></div>
		         </li>      
		...
		?>
		      ...
		      <?php _e('Oops, there are no posts.','fifteen'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'fifteen' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'fifteen' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','fifteen' ); ?></label> 
		<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo $this->get_field_id( 'no_of_posts' ); ?>"><?php _e( 'No. of Posts:', 'fifteen' ); ?></label>
	...
	public function update( $new_instance, $old_instance ) {
		$instance = array();
		$instance['title'] = ( ! empty( $new_instance['title'] ) ) ? wp_strip_all_tags( $new_instance['title'] ) : 'fifteen';
...
add_action( 'widgets_init', 'fifteen_register_widget' );  
function fifteen_register_widget() {  
    register_widget( 'fifteen_Recent_Posts' );  
}  
