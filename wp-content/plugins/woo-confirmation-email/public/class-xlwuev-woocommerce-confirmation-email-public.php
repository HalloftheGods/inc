...
class XLWUEV_Woocommerce_Confirmation_Email_Public {
	...
	public function __construct() {
		...
		add_action( 'woocommerce_checkout_update_user_meta', array(
			$this,
			'new_user_registeration_from_checkout_form'
		), 10, 2 );
	 ...
	 * @return mixed
	 */
	public function new_user_registration_from_registration_form( $customer, $user_id ) {
	...
	public function custom_form_user_register( $user_id ) {
		...
		if ( ! is_super_admin() && 'administrator' != $user->roles[0] ) {
			if ( 'true' != $status ) {
				if ( false == $this->is_new_user_email_sent ) {
					if ( false == $this->is_checkout_page && false == $this->is_user_made_from_myaccount_page ) {
						$this->is_user_made_from_custom_form = true;
						$this->new_user_registration( $user_id );
						$this->is_new_user_email_sent = true;
					}
				}
	...
	public function redirect_new_user( $redirect ) {
		if ( true == $this->is_new_user_email_sent && false == $this->is_xlwuev_resend_link_clicked && defined( 'WC_DOING_AJAX' ) === false && false == is_order_received_page() ) {
	...
	public function authenticate_user_by_email() {
		...
		if ( isset( $_GET['woo_confirmation_verify'] ) && '' != $_GET['woo_confirmation_verify'] ) {
	...
	public function show_notification_message() {
		if ( isset( $_GET['xlrm'] ) && '' != $_GET['xlrm'] ) {
			XlWUEV_Common::$wuev_user_id = base64_decode( $_GET['xlrm'] );
			...
			if ( 'customer' == $user_role ) {
				...
				if ( false == $order_received_page && false == $this->is_user_verified && '1' == XlWUEV_Common::get_setting_value( 'wuev-general-settings', 'xlwuev_restrict_user' ) ) {
					if ( false == $order_pay_page ) {
						...
						if ( false == $order_received_page ) {
							$redirect_url = add_query_arg( array( 'xlsm' => base64_encode( $user_id ), ), get_the_permalink( $this->my_account ) );
		...
		if ( isset( $_GET['xlsm'] ) && '' != $_GET['xlsm'] ) {
			XlWUEV_Common::$wuev_user_id = base64_decode( $_GET['xlsm'] );
			if ( false === WC()->session->has_session() ) {
				WC()->session->set_customer_session_cookie( true );
			}
	...
	public function new_user_registration( $user_id ) {
		...
		$this->code_mail_sender( $current_user->user_email );
		$this->is_new_user_email_sent = true;
			...
			if ( true != get_user_meta( $user->ID, 'wcemailverified', true ) ) {
				...
				if ( '1' == $is_force_login_enabled ) {
					...
					if ( false == is_order_received_page() && false == $this->is_checkout_page ) {
						$redirect_url = add_query_arg( array( 'xlsm' => base64_encode( $user->ID ), ), get_the_permalink( $this->my_account ) );
						wp_redirect( $redirect_url );
						exit;
					}
				} else {
	...
	public function please_confirm_email_message( $user_id ) {
		...
		if ( false == $this->is_user_verified ) {
	...
	public function please_login_email_message() {
		...
		if ( get_user_meta( XlWUEV_Common::$wuev_user_id, 'wcemailverified', 'true' ) && $this->is_user_already_verified ) {
			...
			if ( '1' == XlWUEV_Common::get_setting_value( 'wuev-general-settings', 'xlwuev_verification_page' ) ) {
				...
				$hyperlink = add_query_arg( array( 'xlvm' => true ), get_the_permalink( XlWUEV_Common::get_setting_value( 'wuev-general-settings', 'xlwuev_verification_page_id' ) ) );
				wp_redirect( $hyperlink );
				exit();
			}
		}
	}
	...
	public function resend_verification_email() {
		if ( isset( $_GET['wc_confirmation_resend'] ) && '' != $_GET['wc_confirmation_resend'] ) {
			$user_id                               = base64_decode( $_GET['wc_confirmation_resend'] );
			XlWUEV_Common::$wuev_user_id           = $user_id;
			XlWUEV_Common::$wuev_myaccount_page_id = $this->my_account;
			$this->is_xlwuev_resend_link_clicked   = true;
			$this->new_user_registration( $user_id );
	...
	public function wc_email_verification_code() {
		...
		$hyperlink   = add_query_arg( array( 'woo_confirmation_verify' => base64_encode( $create_link ) ), get_the_permalink( $this->my_account ) );
	...
	public function wuev_shortcode_xlwuev_verification_page() {
		if ( isset( $_GET['xlvm'] ) && '' != $_GET['xlvm'] ) {
			...
			wc_add_notice( __( XlWUEV_Common::get_setting_value( 'wuev-messages', 'xlwuev_email_success_message' ), 'wc-email-confirmation' ), 'notice' );
		}
	}
	...
	public function code_mail_sender( $email ) {
		...
		if ( '1' == $verification_email_template ) {
			...
			$email_body = apply_filters( 'the_content', $message );
		} elseif ( '2' == $verification_email_template ) {
			$email_body = apply_filters( 'the_content', $email_body );
			if ( '1' == $verification_email_type || $this->is_xlwuev_resend_link_clicked ) {
				$mailer = WC()->mailer();
				ob_start();
				...
				$email_body            = apply_filters( 'woocommerce_mail_content', $email_abstract_object->style_inline( wpautop( wptexturize( $email_body ) ) ) );
			}
		}

		if ( '2' == $verification_email_template && '2' == $verification_email_type && true == $this->is_user_made_from_custom_form ) {
		...
		if ( '1' == $verification_email_template || $this->is_xlwuev_resend_link_clicked || true == $this->is_user_made_from_custom_form ) {
			$result = wp_mail( $email, $email_subject, $email_body, $header );
		} else if ( '2' == $verification_email_template && '1' == $verification_email_type ) {
			$result = wp_mail( $email, $email_subject, $email_body, $header );
		}
