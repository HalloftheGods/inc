...
if ( ! class_exists( 'XL_API' ) ) :
	...
	class XL_API {
		...
		public static function post_tracking_data( $data ) {
			...
			$api_params = self::get_api_args( array( 'edd_action' => 'get_tracking_data', 'data' => $data ) );
		 ...
		 * @return type
		 */
		public static function post_optin_denied_data( $data ) {

			if ( empty( $data ) ) {
				return;
			}

			$api_params = self::get_api_args( array( 'edd_action' => 'get_optin_denied_data', 'data' => $data ) );
		...
		public static function post_support_request( $data ) {
			...
			$api_params = self::get_api_args( array( 'edd_action' => 'submit_support_request', 'data' => $data ) );
		...
		/**
		 * Filter functin for request args
		 *
		 * @param type $args
		 *
		 * @return type
