...
/**
 * This class contains status logics that makes product grid actionable
 * @package XLCore
 * @since 1.0.0
 * @author XLPlugins
 */
class XL_addon extends PW_Plugin_states
{
     ...
     * <br/> Varify several configurations so that it wont create issues further.
     * <br/> Perepe cases (imp) Whole logic to decide plugins state.
    ...
    public function populate_properties()
    {
        ...
        if ($this->config && is_object($this->config)) {
            ...
            $this->buynow_link = $this->config->$current_slug->buy_now_link;
    ...
    public function parse_terms()
    {
        ...
        if ($this->config && is_object($this->config)) {
            ...
            if ($this->config->$current_slug->terms && count($this->config->$current_slug->terms) > 0) {
                foreach ($this->config->$current_slug->terms as $term => $termobj) {

                    $get_all_slugs = wp_list_pluck($termobj, 'slug');
                    if (!empty($get_all_slugs)) {
                        $get_terms = wp_list_pluck($termobj, 'slug');
    ...
    /**
     * Prepere case
     * As we are handling several scenerios to the dashboard screen so to make grid actions we need to have exact state of a product
