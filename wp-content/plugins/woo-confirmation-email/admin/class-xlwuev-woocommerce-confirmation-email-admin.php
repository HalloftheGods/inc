...
class XLWUEV_Woocommerce_Confirmation_Email_Admin {
	...
	public $is_test_email_sent = false;
	...
	public function show_save_admin_notice() {
		if ( $this->is_plugin_settings_saved ) {
			?>
            <div class="updated fade no-margin-left"><p><b><?php _e( 'Changes Saved', 'wc-email-confirmation' ); ?>
                </p></b>
            </div>
			<?php
		}
	...
	public function bulk_verify_users( $form_values ) {
		...
		$user_email         = array();
		$no_user_text       = __( ' All users are verified for this role.', 'wc-email-confirmation' );
		$single_user_text   = __( ' user is verified.', 'wc-email-confirmation' );
		$multiple_user_text = __( ' users are verified.', 'wc-email-confirmation' );
	...
	public function test_email( $to_email ) {
		...
		$wcep     = new XLWUEV_Woocommerce_Confirmation_Email_Public();
		$wcep->set_user_id( $admin_id );
		$wcep->is_xlwuev_resend_link_clicked = true;
		$wcep->is_test_email                 = true;
		...
		$result                                = $wcep->code_mail_sender( $to_email );
		if ( $result ) {
			$result = __( 'Email Sent Successfully. Please check your Mail Box.', 'wc-email-confirmation' );
		} else {
			$result = __( 'Email not sent.', 'wc-email-confirmation' );
		}

		$this->is_test_email_sent = $result;
	}

	public function test_email_success_admin() {
		if ( false != $this->is_test_email_sent ) {
			?>
            <div class="updated notice">
                <p><?php echo $this->is_test_email_sent; ?></p>
            </div>
			<?php
		}
	}
	...
	public function create_tabs( $tabs, $current = null ) {
		...
		foreach ( $tabs as $location => $tabname ) {
			...
			$content .= '<a class="nav-tab' . $class . '" href="?page=wc-email-confirmation&tab=' . $location . '">' . $tabname . '</a>';
		}
		$content .= '</h2>';

		switch ( $current ) {
			case 'wuev-bulk-verification':
				$submit_button_text = __( 'Verify All', 'wc-email-confirmation' );
				break;
			case 'wuev-test-email':
				$submit_button_text = __( 'Send Test Email', 'wc-email-confirmation' );
				break;
			default:
				$submit_button_text = __( 'Save Changes', 'wc-email-confirmation' );
				break;
		}
	...
	public function update_user_table( $column ) {
		$column['wuev_verified']            = __( 'Verification Status', 'wc-email-confirmation' );
		$column['wuev_manual_verified']     = __( 'Manual Verify', 'wc-email-confirmation' );
		$column['wuev_manual_confirmation'] = __( 'Confirmation Email', 'wc-email-confirmation' );
		...
		if ( 'wuev_verified' == $column_name ) {
			...
			if ( 'administrator' != $user_role->roles[0] ) {
				if ( get_user_meta( $user_id, 'wcemailverified', true ) == 'true' ) {
					return '<span class="wuev-circle wuev-iconright" title="' . __( 'Verified', 'wc-email-confirmation' ) . '"></span>';
				} else {
					return '<span class="wuev-circle wuev-iconwrong" title="' . __( 'Not Verified', 'wc-email-confirmation' ) . '"></span>';
				}
			} else {
				return 'Admin';
			}
			...
			if ( 'administrator' != $user_role->roles[0] ) {
				if ( get_user_meta( $user_id, 'wcemailverified', true ) != 'true' ) {
					...
					$text = __( 'Verify This', 'wc-email-confirmation' );
					...
					return '<a href=' . add_query_arg(
						...
						) . '>' . apply_filters( 'wc_email_confirmation_manual_verify', $text ) . '</a>';
				}
			}
		} else if ( 'wuev_manual_confirmation' == $column_name ) {
			if ( 'administrator' != $user_role->roles[0] ) {
				$text = __( 'Send Email', 'wc-email-confirmation' );
				...
				return '<a href=' . add_query_arg(
					...
					) . '>' . apply_filters( 'wc_email_confirmation_manual', $text ) . '</a>';
			}
		}
	...
	public function manual_verify_user() {
		...
		if ( isset( $_GET['user_id'] ) && isset( $_GET['wp_nonce'] ) && wp_verify_nonce( $_GET['wp_nonce'], 'wc_email_confirmation' ) ) {
			$wcep = new XLWUEV_Woocommerce_Confirmation_Email_Public();
			$wcep->set_user_id( $_GET['user_id'] );
			$wcep->is_xlwuev_resend_link_clicked = true;
			$current_user                        = get_user_by( 'id', $_GET['user_id'] );
			$is_secret_code_present              = get_user_meta( $_GET['user_id'], 'wcemailverifiedcode', true );
			if ( '' == $is_secret_code_present ) {
				$secret_code = md5( $_GET['user_id'] . time() );
				update_user_meta( $_GET['user_id'], 'wcemailverifiedcode', $secret_code );
			}
			XlWUEV_Common::$wuev_user_id           = $_GET['user_id'];
			XlWUEV_Common::$wuev_myaccount_page_id = $this->my_account_id;
			$wcep->code_mail_sender( $current_user->user_email );
	...
	public function manual_confirmation_email_success_admin() {
		$text = __( 'Verification Email Successfully Sent.', 'wc-email-confirmation' );
		?>
        <div class="updated notice">
            <p><?php echo $text; ?></p>
        </div>
	...
	public function manual_verify_email_success_admin() {
		$text = __( 'User Verified Successfully.', 'wc-email-confirmation' );
		?>
        <div class="updated notice">
            <p><?php echo $text; ?></p>
        </div>
	...
	public function check_plugin_takenover_notice() {
		...
		if ( 'no' == $is_takenover_dismissed ) {
			$license_invalid_text = sprintf( __( '<p> <a href="%s" target="_blank">XLPlugins</a> has taken over development of Woocommerce User Email Verification.</p><p> We would be pushing new features. Feel free to contact us via <a href="%s" target="_blank">Support ticket </a> to tell us about New ideas or just to say simple Hi :)</p><p><a href="%s" class="button">Dismiss this notice</a></p>', XLWUEV_TEXTDOMAIN ), 'https://xlplugins.com/', admin_url( 'admin.php?page=xlplugins&tab=support' ), esc_url( add_query_arg( 'xl-take-over-notice-dismiss', 'yes' ) ) );
			?>
	...
	public function check_server_email_capability() {
		...
		if ( 'no' == $is_check_server_email_dismissed ) {
			$display_text = sprintf( __( '<p> You can test to see if your server is capable of sending Emails.</p><p><a href="%s" class="button button-primary">Yes, I want to test</a> <a href="%s" class="button">No, I don\'t want to test</a></p>', XLWUEV_TEXTDOMAIN ), admin_url( 'admin.php?page=wc-email-confirmation&tab=wuev-test-email' ), esc_url( add_query_arg( 'xl-server-email-capability-dismiss', 'yes' ) ) );
			?>
            <div id="message" class="notice notice-info">
...
//				'role'       => $user_role,
			...
			if ( $total_unverified_users > 0 ) {
				$display_text = sprintf( __( '<p> Your website contains %s users whose emails are not verified.</p><p><a href="%s" class="button button-primary">I want to verify</a> <a href="%s" class="button">No, I don\'t want to verify</a></p>', XLWUEV_TEXTDOMAIN ), $total_unverified_users, admin_url( 'admin.php?page=wc-email-confirmation&tab=wuev-bulk-verification' ), esc_url( add_query_arg( 'xl-old-users-check', 'yes' ) ) );
				?>
                <div id="message" class="notice notice-info">
					<?php echo $display_text; ?>
                </div>
	...
	public function insert_plugin_takenover_notice() {
		if ( isset( $_GET['xl-take-over-notice-dismiss'] ) && 'yes' == $_GET['xl-take-over-notice-dismiss'] ) {
	...
	public function insert_server_email_capability_notice() {
		if ( isset( $_GET['xl-server-email-capability-dismiss'] ) && 'yes' == $_GET['xl-server-email-capability-dismiss'] ) {
	...
	public function insert_old_users_notice() {
		if ( isset( $_GET['xl-old-users-check'] ) && 'yes' == $_GET['xl-old-users-check'] ) {
