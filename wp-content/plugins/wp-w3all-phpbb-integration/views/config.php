...
$w3all_conf_pref['w3all_profile_sync_bp_yn'] = isset($w3all_conf_pref['w3all_profile_sync_bp_yn']) ? $w3all_conf_pref['w3all_profile_sync_bp_yn'] : 0;
 ...
 $config_file =  $_POST["w3all_conf"]["w3all_path_to_cms"] . '/config.php';
    ...
    if ( !defined('PHPBB_INSTALLED') ){
     echo __('<h3 style="color:#ff0000">Before to activate the integration by setting the path to a phpBB <i>config.php</i> file it is mandatory to<br /><br /> <a target="_blank" href="https://www.axew3.com/w3/2016/02/configure-phpbb-for-cookie-on-subdomains/">setup the correct cookie setting into phpBB</a> (and read the <a target="_blank" href="https://www.axew3.com/w3/cms-plugins-scripts/wordpress-plugins-scripts-docs/wordpress-phpbb-integration/">Install Help Page</a>)</h3>', 'wp-w3all-phpbb-integration');
     ...
     echo __('<h4 style="color:#ff0000">Notice: <span style="color:#000">WP_w3all for subdomains installations.</span> Use the manual config option. If you choose to include/use the default phpBB config.php file that reside into real phpBB folder, and result impossible to correctly include the phpBB config.php file (so this red message still display) read this <a target="_blank" href="https://www.axew3.com/w3/2016/03/how-to-disable-open_basedir-for-subdomain-file-inclusion/">post about files inclusions restrictions</a>.</h4>', 'wp-w3all-phpbb-integration');
     }
...
<br />Path example to phpBB root folder, for config.php on phpBB root folder: <i>/web/htdocs/home/<b>phpBB</b></i>
...
<?php echo __('If some forum require specific permissions to be viewed and user not belong to this specific group, posts/topics from these forums are not retrieved to be displayed into Last Topics Widgets.', 'wp-w3all-phpbb-integration'); ?>
...
<div class=""><?php echo __('<h3>Use custom files to display Last Topics Widgets, Login Widget or Last Topics Shortcode content</h3>', 'wp-w3all-phpbb-integration'); ?></div>
<?php echo __('Files that display widgets output and that reside into folder <i>/wp-content/plugins/wp-w3all-phpbb-integration<b>/views</b></i><br />which are <i><b>phpbb_last_topics.php, phpbb_last_topics_output_shortcode.php, login_form_include_noiframe_mode_links.php</b> and <b>login_form_include_iframe_mode_links.php</b></i><br />can be copied/pasted into the custom folder (that you may have already manually create, if you have choose to include/use the custom phpBB <i>config.php</i> file):<br /><i>/wp-content/plugins<b>/wp-w3all-config</b></i><br />custom modifications done into these files, aren\'t overwritten when plugin update.<br /><br /><b>NOTE:</b> if you activate this option, <b>it is mandatory</b> that you copy/paste all these four (4) files into the custom <i>/wp-content/plugins/<b>wp-w3all-config</b></i> folder.', 'wp-w3all-phpbb-integration'); ?>
<p><label""><input type="radio" name="w3all_conf_pref[w3all_custom_output_files]" id="w3all_custom_output_files_1" value="1" <?php checked('1', $w3all_conf_pref['w3all_custom_output_files']); ?> /> <?php echo __('Yes, use custom files that i\'ve copy into <i>/wp-content/plugins/<b>wp-w3all-config</b></i> folder', 'wp-w3all-phpbb-integration'); ?></label></p>
<p><label""><input type="radio" name="w3all_conf_pref[w3all_custom_output_files]" id="w3all_custom_output_files_0" value="0" <?php checked('0', $w3all_conf_pref['w3all_custom_output_files']); ?> /> <?php echo __('No, use default folder\'s plugin files', 'wp-w3all-phpbb-integration'); ?></label></p>
...
<?php echo __('<br /><!--<span style="text-decoration: line-through;"><b>Note</b>: this work only with default WP registration system where WP send an email link to set first user\'s password, that user do not know at this time. If you have install an external registration plugin that let choose the password to the user on register, than this option may will not affect. If your registration plugin provide option to let choose password or not on register for users, than set no the option, and all here should work as expected about WP/phpBB account confirmation/activation.</span>-->', 'wp-w3all-phpbb-integration');?></div>
...
<hr />
<div class=""><?php echo __('<h3>Activate language update/switch on profile for users between WordPress and phpBB</h3>When user change language on profile, it will be so updated also on phpBB/WP. When user register, the active used WP default language will be set for the user in phpBB.<br /><strong>Note: if same language not exist</strong> installed also into phpBB, (ex. an user switch on his WP profile to a language available into WordPress, but that has not been installed into phpBB) phpBB may will return error for this user on certain situations (on send out a PM for example)', 'wp-w3all-phpbb-integration'); ?></div>
...
<hr />
<div class=""><?php echo __('<h3 style="color:#da5252">Synchronize Buddypress/phpBB user\'s profile fields</h3>Before to activate this option (that is not mandatory and affect only if Buddypress is installed) read the <b>How To</b> here: <a target="_blank" href="https://www.axew3.com/w3/2017/09/wordpress-and-buddypress-phpbb-profile-fields-integration/">Buddypress/phpBB profile fields how to</a>', 'wp-w3all-phpbb-integration'); ?></div>
<p><label""><input type="radio" name="w3all_conf_pref[w3all_profile_sync_bp_yn]" id="w3all_profile_sync_bp_yn_1" value="1" <?php checked('1', $w3all_conf_pref['w3all_profile_sync_bp_yn']); ?> /> <?php echo __('Yes', 'wp-w3all-phpbb-integration'); ?></label></p>
<p><label""><input type="radio" name="w3all_conf_pref[w3all_profile_sync_bp_yn]" id="w3all_profile_sync_bp_yn_0" value="0" <?php checked('0', $w3all_conf_pref['w3all_profile_sync_bp_yn']); ?> /> <?php echo __('No', 'wp-w3all-phpbb-integration'); ?></label></p>
<hr />
<div class=""><?php echo __('<h3>Activate WordPress to phpBB and phpBB to WP users transfer</h3><strong>NOTE: it is mandatory</strong>, as explained on <a target="_blank" href="https://www.axew3.com/w3/cms-plugins-scripts/wordpress-plugins-scripts-docs/wordpress-phpbb-integration/">Help Install Page</a>, <strong>to transfer existent WordPress users into phpBB when integration start!</strong><br />
	While <strong>it is not mandatory</strong>, to transfer phpBB users into WordPress when integration start.<br /><br />Once activated it will be visible in WordPress admin side menu under Settings Menu: when transfer has been finished, turn it off (and remove from admin side Settings Menu).', 'wp-w3all-phpbb-integration'); ?></div>
...
<hr />
<div class=""><?php echo __('<h3>Use phpBB avatar to replace WordPress user\'s avatar</h3>If set to Yes, Gravatars profiles images on WordPress, are replaced by phpBB user\'s avatars images, where an avatar image is available in phpBB for the user. Return WP Gravatar of the user, if no avatar image has been found in phpBB (one single fast query to get avatars for all users).
...
<br /><b>Note</b> about .htaccess: <b>"i\'ve setup avatar here, but avatar\'s images aren\'t displayed!" ... <a href="https://www.axew3.com/w3/2016/09/phpbb-htaccess-set-avatars-images-available-over-your-domain/" target="_blank">take a look to this <i>why and how to resolve</i> post</a></b> before you activate this feature.
<br />Check that on <i>WordPress Admin -> Settings -> Discussion</i> the setting about avatars is enabled. Check also that isn\'t set to BLANK this setting (if you do not want really it).<br />Note: a (may still) not added phpBB user in WordPress, will display default Gravatar until it isn\'t added also in WP side.', 'wp-w3all-phpbb-integration'); ?>
...
<div class=""><?php echo __('<h3 style="color:#869eff">Activate phpBB avatars on Last Forums Topics widgets</h3>Add avatars for each user on Last Forums Topics widget', 'wp-w3all-phpbb-integration'); ?></div>
...
<hr />
<div class=""><?php echo __('<h3>Last Forums Topics Widget avatar\'s dimension</h3>Set the avatar dimension (in pixel) for Last Forum Topics Widget (Ex: 50).<br />Note: affect only if the above <i style="color:#869eff">Activate phpBB avatars on Last Forums Topics widgets</i> option is set to yes', 'wp-w3all-phpbb-integration'); ?></div>
...
<hr />
<div class=""><?php echo __('<h3 style="color:#da5252">Replace Buddypress profile avatar with phpBB user avatar (and vice versa)</h3><b>This option affect only if Buddypress is installed into your WordPress.</b> If activated, the Buddypress profile user\'s avatar will be replaced by the phpBB user\'s avatar, where available: if not, the default avatar of the user in WP will be retrieved.<br /><strong>Note</strong>: to have the uploaded Buddypress user\'s avatar shown into phpBB, phpBB need to have set to YES the option <b>"Enable remote avatars"</b> in order to this feature to work correctly<br /> ACP -> Board Configuration -> Avatar settings<br /><br />Note: affect only if the above <i style="color:#869eff">Use phpBB avatar to replace WordPress user\'s avatar</i> option is set to yes', 'wp-w3all-phpbb-integration'); ?></div>
<p><label""><input type="radio" name="w3all_conf_avatars[w3all_avatar_replace_bp_yn]" id="w3all_avatar_replace_bp_yn_1" value="1" <?php checked('1', $w3all_config_avatars['w3all_avatar_replace_bp_yn']); ?> /> <?php echo __('Yes', 'wp-w3all-phpbb-integration'); ?></label></p>
<p><label""><input type="radio" name="w3all_conf_avatars[w3all_avatar_replace_bp_yn]" id="w3all_avatar_replace_bp_yn_0" value="0" <?php checked('0', $w3all_config_avatars['w3all_avatar_replace_bp_yn']); ?> /> <?php echo __('No', 'wp-w3all-phpbb-integration'); ?></label></p>
...
<br />Note: you can completely ignore this part about using iframe mode and use wp_w3all without embed phpBB template in a WP page.</h4></div>
